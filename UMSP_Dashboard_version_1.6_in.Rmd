...
setwd("D:/UMSP/UMSP_Data_Dashboard")
...
# Load data  
UMSP_data <- read.dta13("D:/UMSP/UMSP_Data_Dashboard/Monthly data for all sites through December 2024.dta",   
...
UMSP_data <- UMSP_data %>%  
  ...
  mutate(  
    Month = format(monthyear, "%B"),  # Extract Month  
    MonthYearFormatted = format(monthyear, "%B")  # Month 
  )
  ...
  sidebarLayout(  
    sidebarPanel(  
      ...
      selectInput("month", "Select Month(s):",  
                  choices = c("All Months", unique(UMSP_data$Month)),  
                  selected = "All Months",  
                  multiple = TRUE),
...
server <- function(input, output) {  
  filtered_data <- reactive({  
    ...
    if (!"All Months" %in% input$month) {  
      data <- data %>% filter(Month %in% input$month)  
    } 
    return(data)  
  })  
